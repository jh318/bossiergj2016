...
public class BorderSpawn : MonoBehaviour {
	...
	void Update () {
		...
		if (Time.frameCount%360.0f == 0){ //check if total frame count is divisiable by 360 frames IE every 6 seconds, spawn enemy
			Instantiate(enemy, new Vector3(borders[b].transform.position.x, borders[b].transform.position.y, 0), Quaternion.identity); //create instance of object at this position
		}
